...
const kelasGuruToDelete = ref(null);

const confirmDelete = (kelasguru) => {
    kelasGuruToDelete.value = kelasguru;
    showDeleteModal.value = true;
};

const deleteKelasGuru = async () => {
    if (!kelasGuruToDelete.value) return;

    try {
        await router.delete(
            route('kelas-guru.delete', kelasGuruToDelete.value.id),
        );
        showDeleteModal.value = false;
    } catch (error) {
        console.error('Error mendelete kelas guru siswa:', error);
    }
};
</script>
                ...
                >
                    <div class="flex items-center justify-between">
                        <div class="flex flex-1 justify-between sm:hidden">
                            <a
                                v-if="kelasguru.prev_page_url"
                                :href="kelasguru.prev_page_url"
                                class="relative inline-flex items-center rounded-md border border-gray-300 bg-white px-4 py-2 text-sm font-medium text-gray-700 hover:bg-gray-50"
                            >
                                Previous
                            </a>
                            <a
                                v-if="kelasguru.next_page_url"
                                :href="kelasguru.next_page_url"
                        ...
                        >
                            <div>
                                <p class="text-sm text-gray-700">
                                    Showing {{ kelasguru.from }} to
                                    {{ kelasguru.to }} of
                                    {{ kelasguru.total }} results
                                </p>
                            </div>
                            <div>
                                <nav
                    ...
                    <div class="mt-3 text-center">
                        ...
                        <div class="mt-2 px-7 py-3">
                            <p class="text-sm text-gray-500">
                                Apakah Anda yakin ingin menghapus kelas guru
                                siswa "{{ kelasGuruToDelete?.nama }}"? Tindakan
